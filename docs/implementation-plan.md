...
| G2 | 최초 설정 | "기본 예산으로 생성" 버튼 하나 | 항목 선택 → 예산 입력 → 총합 실시간 → 최종 확인 | 4단계 |
...
| G13 | 기본 항목 | 개인용 7개 (`평일 데이트` 등) | 기획서 추천 9개 | 4단계 |
...
- 총합 실시간 표시, 최소 1개 항목 검증, 최종 확인 후 일괄 커밋. 검증은 `buildOnboardingPlan` 하나.

### 5단계 — 홈 화면 정리
- 전체 생활비 카드를 기획서 §7.3 문구·정보 우선순위에 맞춘다(`STATUS_MESSAGE`를 여기서 쓴다).
